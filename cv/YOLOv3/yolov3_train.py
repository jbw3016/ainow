...
def main():
    ...
    def train_step(image_data, target):     # 학습 관련 핵심 함수
        with tf.GradientTape() as tape:
            ...
            for i in range(grid):
                ...
                # 모델의 출력 예측값과 타겟값을 compute_loss() 함수에 전달해서
                # 해당 loss를 게산
            ...
            # 계산된 total loss 값을 이용하여 모델 가중치를 오차 역전파해서
            # 업데이트 시킴
    ...
    for epoch in range(TRAIN_EPOCHS):   # 몇 번 반복학습을 진행할 것인지 epochs 설정
        count, giou_val, conf_val, prob_val, total_val = 0, 0, 0, 0, 0

        for image_data, target in trainset:  # for() 한번 동작 시 마다 --> __next__(self) 동작
            results = train_step(image_data, target)    # 함수 호출
            count += 1
