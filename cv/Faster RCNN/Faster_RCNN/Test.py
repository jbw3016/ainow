...
class TestCls():
    ...
    def Test_start(self):
        ...
        for i in range(len(rcnn_classifier)):
            ...
            findClassScorelist.append(rcnn_classifier[i][1])
            findRegBoxlist.append([xmin, ymin, xmax, ymax])
            # _, ax = plt.subplots(1)
        ...
        if len(findClassScorelist) > 0:
            maxidx = np.argmax(findClassScorelist)
            print('maxidx : ', maxidx)
            box = findRegBoxlist[maxidx]
            x_min = int(box[0])
            y_min = int(box[1])
            x_max = int(box[2])
            y_max = int(box[3])
