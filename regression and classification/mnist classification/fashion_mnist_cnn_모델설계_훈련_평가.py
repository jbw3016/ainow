...
# print(len(train_input),train_input.shape) # 모델 학습 용도로 사용
# print(len(test_input),test_input.shape) # 모델 평가 용도로 사용
...
train_scaled = train_input.reshape(-1,28,28,1) / 255.0  # 이미지 데이터 정규화 (높이, 너비, 채널)
...
model.add(Dropout(0.4))
    # Dropout(rate) : 학습 중에 랜덤으로 선택된 뉴런의 출력을 0으로 만들어, 뉴런이 학습에 기여하는 정도를 조절

#출력층 , 소프트맥스 활성화 함수
model.add(Dense(10,activation='softmax'))
    # 분류할 label 갯수와 마지막 layer의 뉴런 갯수는 맞추어야.
