...
print(f"train_input.shape : {train_input.shape}") # 60000
...
from sklearn.model_selection import train_test_split    #  훈련 세트와 검증 세트로 분할
...
#예측
preds = CNN_NewModel.predict(val_scaled)    # 검증 데이터에 대한 예측값
print(f"preds.shape : {preds.shape}\n")
print(f"preds[0] :\n{preds[0]}\n")  # 검증 데이터로 예측한 첫번째 결과값
pred_label = np.argmax(preds[0])  # 예측 결과값에 대한 최대 예측값의 인덱스 추출
print(f"prred_label : {pred_label}")
print(f"val_target[0] : {val_target[0]}\n") # 검증 데이터의 타깃값 확인
...
img_path = r'C:\Users\Harmony25\Desktop\DL_2\new\sandal_1.jpg'
